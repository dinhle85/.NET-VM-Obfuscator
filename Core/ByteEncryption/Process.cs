...
namespace Core.ByteEncryption
{
    class Process
    {
        ...
        public static byte[] tester(MethodDef methodDef, ModuleDefMD updated)
        {
            ...
            var streamFull = updated.MetaData.PEImage.CreateFullStream();
        ...
        public unsafe static void processConvertedMethods(List<MethodData> allMethodDatas)
        {
            ...
            var modopts = new ModuleWriterOptions(Protector.moduleDefMD);
            ...
            var updated = ModuleDefMD.Load(tester);
            ...
            foreach (MethodData methodData in allMethodDatas)
            {
                ...
                //  伊诶哦西勒西艾.a(enc, enc.Length, methodBytes, methodBytes.Length);
        ...
        static byte[] b(byte[] toEncrypt, int len)
        {

            var key = "HCP"; //Any chars will work, in an array of any size
        ...
        static byte[] aMethod2(byte[] data, int datalen, byte[] key, int keylen)
        {
            var N1 = 125;
            var N2 = 14;
            var NS = 258;
        ...
        public static byte[] Bmethod(byte[] toEncrypt, int len)
        {
            ...
            char[] key = { '西', 'C', 'ض', '۞', 'Ұ', '۝', '۞', 'Ұ', 'Ұ' };
            byte[] output = toEncrypt;
            for (int i = 0; i < len; i++)
                output[i] = (byte)(toEncrypt[i] ^ key[i % (9 / sizeof(char))]);
            return output;
        }
    }
}