...
{
    public class 伊诶哦西勒西艾
    {
        public static object locker2 = new object();
        public static object 西勒西艾娜屁西艾伊艾(int position, int size, int iD, object[] parameters)
        {
            ...
            {
                var callingMethod = new StackTrace().GetFrame(1).GetMethod();//get calling method, using this can prevent invocation and dynamic unpacking
                var grabbedBytes = byteArrayGrabber(哦勒艾艾伊吉.byteArrayResource, position, size);
                ...
                哦勒艾艾伊吉.bc(grabbedBytes, grabbedBytes.Length, ab, ab.Length);
        ...
        {
            var methodBody = callingMethod.GetMethodBody();//get calling methods body 
            var binaryReader = new BinaryReader(new MemoryStream(bytes));//cast byte[] Position
            ...
            var dynamicMethod = new DynamicMethod("", callingMethod.MemberType == MemberTypes.Constructor ? null : ((MethodInfo)callingMethod).ReturnParameter.ParameterType, parametersArray, 哦勒艾艾伊吉.callingModule, true);//create the dynamic method
            var ilGenerator = dynamicMethod.GetILGenerator();//get ilgenerator
            ...
            for (var i = 0; i < instructionCount; i++)
            {
                ...
                OpCode opc;
                if (opcode >= 0 && opcode < 哦勒艾艾伊吉.oneByteOpCodes.Length)
                {
                    opc = 哦勒艾艾伊吉.oneByteOpCodes[opcode];//we check against one byte opcodes
                }
                else
                {
                    var b2 = (byte)(opcode | 0xFE00);
                    opc = 哦勒艾艾伊吉.twoByteOpCodes[b2];//check against two byte opcodes
                }
        ...
        static void HandleOpType(int opType, OpCode opcode, ILGenerator ilGenerator, BinaryReader binaryReader, Dictionary<int, Label> _allLabelsDictionary, List<LocalBuilder> allLocals)
        {
        ...
        /// <param name="opcode"></param>
        /// <param name="binaryReader"></param>
        static void InlineNoneEmitter(ILGenerator ilGenerator, OpCode opcode, BinaryReader binaryReader)
        {
        ...
        /// <param name="opcode"></param>
        /// <param name="binaryReader"></param>
        static void InlineMethodEmitter(ILGenerator ilGenerator, OpCode opcode, BinaryReader binaryReader)
        {
            var mdtoken = binaryReader.ReadInt32();
            var resolvedMethodBase = 哦勒艾艾伊吉.callingModule.ResolveMethod(mdtoken);
        ...
        /// <param name="binaryReader"></param>
        /// <param name="allLocals"></param>
        static void InlineVarEmitter(ILGenerator ilGenerator, OpCode opcode, BinaryReader binaryReader, List<LocalBuilder> allLocals)
        {
        ...
        /// <param name="opcode"></param>
        /// <param name="binaryReader"></param>
        static void InlineStringEmitter(ILGenerator ilGenerator, OpCode opcode, BinaryReader binaryReader)
        {
            var readString = binaryReader.ReadString();
            ilGenerator.Emit(opcode, readString);
        }

        static void InlineIEmitter(ILGenerator ilGenerator, OpCode opcode, BinaryReader binaryReader)
        {
        ...
        static void InlineFieldEmitter(ILGenerator ilGenerator, OpCode opcode, BinaryReader binaryReader)
        {
            var mdtoken = binaryReader.ReadInt32();
            var fieldInfo = 哦勒艾艾伊吉.callingModule.ResolveField(mdtoken);
            ilGenerator.Emit(opcode, fieldInfo);
        }

        static void InlineTypeEmitter(ILGenerator ilGenerator, OpCode opcode, BinaryReader binaryReader)
        {
            var mdtoken = binaryReader.ReadInt32();
            var type = 哦勒艾艾伊吉.callingModule.ResolveType(mdtoken);
            ilGenerator.Emit(opcode, type);
        }

        static void ShortInlineBrTargetEmitter(ILGenerator ilGenerator, OpCode opcode, BinaryReader binaryReader, Dictionary<int, Label> _allLabelsDictionary)
        {
            var index = binaryReader.ReadInt32();
        ...
        static void ShortInlineIEmitter(ILGenerator ilGenerator, OpCode opCode, BinaryReader binaryReader)
        {
            var b = binaryReader.ReadByte();
            ilGenerator.Emit(opCode, b);
        }

        static void ShortInlineREmitter(ILGenerator ilGenerator, OpCode opCode, BinaryReader binaryReader)
        {
        ...
        static void InlineREmitter(ILGenerator ilGenerator, OpCode opCode, BinaryReader binaryReader)
        {
        ...
        static void InlineI8Emitter(ILGenerator ilGenerator, OpCode opCode, BinaryReader binaryReader)
        {
        ...
        static void InlineSwitchEmitter(ILGenerator ilGenerator, OpCode opCode, BinaryReader binaryReader, Dictionary<int, Label> _allLabelsDictionary)
        {
        ...
        static void InlineBrTargetEmitter(ILGenerator ilGenerator, OpCode opcode, BinaryReader binaryReader, Dictionary<int, Label> _allLabelsDictionary)
        {
            var index = binaryReader.ReadInt32();
            var location = _allLabelsDictionary[index];
            ilGenerator.Emit(opcode, location);
        ...
        static void InlineTokEmitter(ILGenerator ilGenerator, OpCode opcode, BinaryReader binaryReader)
        {
            var mdtoken = binaryReader.ReadInt32();
            ...
            {
                var typeInfo = 哦勒艾艾伊吉.callingModule.ResolveType(mdtoken);
                ilGenerator.Emit(opcode, typeInfo);
            }
            else if (type == 2)
            {
                var methodinfo = 哦勒艾艾伊吉.callingModule.ResolveMethod(mdtoken);
